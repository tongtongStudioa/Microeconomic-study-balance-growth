...
class BalancingSimulation:
    ...
    def analyse_simulations(self):
        ...
        return
class BankAccount:
    def __init__(self,initial_balance = 500,initial_saving = 250, initial_investment = 100,monthly_income = 2500):
    ...
    def init_parameters(self):
        ...
        # Signaler que les paramètres essentiels sont mis à jour
        self.init_param = True
        
    def __str__(self):
        print(f"\nMonthly income : {self.monthly_income} $")
        print(f"Monthly expenses : {self.monthly_expenses} $")
        print(f">>> Capital initial : {self.capital:.1f} $ <<<\n\n")
                
    def simulation(self,months_n,investment_return_rate = 0.05,uncertainty = 0.2,saving_loss_prob = 0.01,investment_loss_prob = 0.15):
        ...
        for i in range(months_n):
            ...
            rate = np.random.uniform(-uncertainty,uncertainty) # Ajoute l'incertitude aux revenus et dépenses
            ...
            if (new_balance > 0):
                ...
                monthly_saving = new_balance * self.saving_ratio * choice([0,1], 1, p=[0.01,0.99])[0] #1 % de chance de perte 
            ...
            if (new_balance > 0 and new_balance * self.investment_ratio > 100):
                monthly_investment = new_balance * self.investment_ratio * choice([0,1], 1, p=[0.15,0.85])[0] #15 % de chance de perte 
